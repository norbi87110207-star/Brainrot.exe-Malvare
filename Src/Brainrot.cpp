...
DWORD WINAPI GdiEffect(LPVOID param)
{
    ...
    while (true)
    {
        ...
        case 1: // Random rectangles
            ...
            break;
        case 2: // Random ellipses
            for (int i = 0; i < 15; i++)
                Ellipse(hdc, rand() % w, rand() % h, rand() % w, rand() % h);
            break;
        case 3: // PolyBezier random
            for (int i = 0; i < 5; i++)
            {
                POINT pts[4];
                for (int j = 0; j < 4; j++)
        ...
        case 4: // Horizontal wave
            for (int y = 0; y < h; y++)
            {
                int x = (int)(sin(a + y * 0.05) * 50);
                BitBlt(hdc, 0, y, w, 1, hdc, x, y, SRCCOPY);
            }
            a += 0.1;
            break;
        case 5: // Vertical wave
            for (int x = 0; x < w; x++)
            {
                int y = (int)(cos(a + x * 0.05) * 50);
                BitBlt(hdc, x, 0, 1, h, hdc, x, y, SRCCOPY);
            }
            a += 0.1;
            break;
        ...
        case 7: // Random triangles
            ...
            break;
        case 8: // Scanline shift
            BitBlt(hdc, 0, 0, w, h, hdc, rand() % 20 - 10, rand() % 20 - 10, SRCCOPY);
            break;
        case 9: // PatBlt stripes
            for (int i = 0; i < 10; i++)
                PatBlt(hdc, rand() % w, rand() % h, 50, 50, PATINVERT);
            break;
        case 10: // Random pixels
            for (int i = 0; i < 200; i++)
                SetPixel(hdc, rand() % w, rand() % h, RGB(rand() % 256, rand() % 256, rand() % 256));
            break;
        case 11: // Zoom rectangle
            StretchBlt(hdc, 10, 10, w - 20, h - 20, hdc, 0, 0, w, h, SRCCOPY);
            break;
        case 12: // Diagonal wave
            for (int y = 0; y < h; y++)
            {
                int x = (int)(sin(a + y * 0.03) * 60);
                BitBlt(hdc, 0, y, w, 1, hdc, x, y, SRCCOPY);
            }
            a += 0.2;
            break;
        case 13: // Random lines
            for (int i = 0; i < 50; i++)
                MoveToEx(hdc, rand() % w, rand() % h, NULL), LineTo(hdc, rand() % w, rand() % h);
            break;
        case 14: // Color scatter
            for (int i = 0; i < 200; i++)
                SetPixel(hdc, rand() % w, rand() % h, RGB(rand() % 256, rand() % 256, rand() % 256));
            break;
        case 15: // PolyBezier small
            for (int i = 0; i < 10; i++)
            {
                POINT pts[4];
                for (int j = 0; j < 4; j++)
        ...
        case 18: // PatBlt + wave
            ...
            for (int y = 0; y < h; y++)
            {
                int x = (int)(cos(a + y * 0.04) * 40);
                BitBlt(hdc, 0, y, w, 1, hdc, x, y, SRCCOPY);
            }
            a += 0.1;
            break;
        case 19: // Random shapes mix
            for (int i = 0; i < 15; i++)
            {
                int shape = rand() % 3;
                if (shape == 0) Rectangle(hdc, rand() % w, rand() % h, rand() % w, rand() % h);
                if (shape == 1) Ellipse(hdc, rand() % w, rand() % h, rand() % w, rand() % h);
                if (shape == 2)
                {
                    POINT pts[3] = { {rand() % w,rand() % h},{rand() % w,rand() % h},{rand() % w,rand() % h} };
                    Polygon(hdc, pts, 3);
...
void RunEffect(int type)
{
    ...
    PlayRandomBytebeat(20); // minden effekt 20 mp
    TerminateThread(t, 0);
    CloseHandle(t);
}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    // 2 warning az elején
    MessageBoxW(NULL, L"Run a Malvare?", L"Brainrot.exe", MB_ICONWARNING);
    MessageBoxW(NULL, L"Are You Sure? it will destroy the computer", L"Brainrot.exe", MB_ICONWARNING);
